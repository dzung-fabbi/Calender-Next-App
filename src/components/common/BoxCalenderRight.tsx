import * as React from 'react'

export default function BoxCalenderRight() {
  return (
    <section className="bg-[#F3F4F3] h-[100vh] py-10 px-5 rounded-l-30px min-w-[25.625rem]">
      <div className="card text-center">
        <p className="text-xl text-left mb-4">Chủ Nhật, 18 Tháng 12, 2022</p>
        <div className="lvn-lichad-lichmain bg-[#FAFBFA] p-3 rounded-2xl">
          <div className="grid gap-2 grid-cols-7 mb-3">
            <div className="font-semibold">Hai</div>
            <div className="font-semibold">Ba</div>
            <div className="font-semibold">Bốn</div>
            <div className="font-semibold">Năm</div>
            <div className="font-semibold">Sáu</div>
            <div className="font-semibold">Bảy</div>
            <div className="font-semibold text-[#FD7770]">CN</div>
          </div>
          <div className="grid gap-2 grid-cols-7">
            <div className="lvn-lichad-col lvn-lichad-colhide">
              <a href="#" title="Xem lịch âm ngày 26 tháng 12 năm 2022">
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span>26</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">4</span>
              </a>
            </div>
            <div className="lvn-lichad-col lvn-lichad-colhide">
              <a href="#" title="Xem lịch âm ngày 27 tháng 12 năm 2022">
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span>27</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">5</span>
              </a>
            </div>
            <div className="lvn-lichad-col lvn-lichad-colhide">
              <a href="#" title="Xem lịch âm ngày 28 tháng 12 năm 2022">
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span>28</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">6</span>
              </a>
            </div>
            <div className="lvn-lichad-col lvn-lichad-colhide">
              <a href="#" title="Xem lịch âm ngày 29 tháng 12 năm 2022">
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span>29</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">7</span>
              </a>
            </div>
            <div className="lvn-lichad-col lvn-lichad-colhide">
              <a href="#" title="Xem lịch âm ngày 30 tháng 12 năm 2022">
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span>30</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">8</span>
              </a>
            </div>
            <div className="lvn-lichad-col lvn-lichad-colhide">
              <a href="#" title="Xem lịch âm ngày 31 tháng 12 năm 2022">
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span>31</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">9</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" holiday"
                href="#"
                title="Xem lịch âm ngày 1 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd lichad-sunday font-semibold text-lg">
                  <span className="day">1</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">10</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 2 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">2</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">11</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 3 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">3</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">12</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 4 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">4</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">13</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 5 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">5</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">14</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" other"
                href="#"
                title="Xem lịch âm ngày 6 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">6</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">15</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 7 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">7</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">16</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 8 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd lichad-sunday font-semibold text-lg">
                  <span className="day">8</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">17</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" other"
                href="#"
                title="Xem lịch âm ngày 9 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">9</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">18</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 10 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">10</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">19</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 11 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">11</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">20</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 12 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">12</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">21</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 13 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">13</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">22</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" other"
                href="#"
                title="Xem lịch âm ngày 14 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">14</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">23</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 15 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd lichad-sunday font-semibold text-lg">
                  <span className="day">15</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">24</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 16 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">16</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">25</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 17 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">17</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">26</span>
              </a>
            </div>
            <div className="lvn-lichad-col today">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 18 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">18</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">27</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 19 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">19</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">28</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 20 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">20</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">29</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" holiday"
                href="#"
                title="Xem lịch âm ngày 21 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">21</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">30</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" holiday"
                href="#"
                title="Xem lịch âm ngày 22 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd lichad-sunday font-semibold text-lg">
                  <span className="day">22</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">
                  1/1
                </span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" holiday"
                href="#"
                title="Xem lịch âm ngày 23 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">23</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">2</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" holiday"
                href="#"
                title="Xem lịch âm ngày 24 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">24</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">3</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 25 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">25</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">4</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 26 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">26</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">5</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 27 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">27</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">6</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 28 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">28</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">7</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 29 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd lichad-sunday font-semibold text-lg">
                  <span className="day">29</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">8</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" "
                href="#"
                title="Xem lịch âm ngày 30 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">30</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">9</span>
              </a>
            </div>
            <div className="lvn-lichad-col">
              <a
                className=" other"
                href="#"
                title="Xem lịch âm ngày 31 tháng 1 năm 2023"
              >
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span className="day">31</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">10</span>
              </a>
            </div>
            <div className="lvn-lichad-col lvn-lichad-colhide">
              <a href="#" title="Xem lịch âm ngày 1 tháng 2 năm 2023">
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span>1</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">11</span>
              </a>
            </div>
            <div className="lvn-lichad-col lvn-lichad-colhide">
              <a href="#" title="Xem lịch âm ngày 2 tháng 2 năm 2023">
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span>2</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">12</span>
              </a>
            </div>
            <div className="lvn-lichad-col lvn-lichad-colhide">
              <a href="#" title="Xem lịch âm ngày 3 tháng 2 năm 2023">
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span>3</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">13</span>
              </a>
            </div>
            <div className="lvn-lichad-col lvn-lichad-colhide">
              <a href="#" title="Xem lịch âm ngày 4 tháng 2 năm 2023">
                <div className="lvn-lichad-dd font-semibold text-lg ">
                  <span>4</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">14</span>
              </a>
            </div>
            <div className="lvn-lichad-col lvn-lichad-colhide">
              <a href="#" title="Xem lịch âm ngày 5 tháng 2 năm 2023">
                <div className="lvn-lichad-dd lichad-sunday font-semibold text-lg">
                  <span>5</span>
                </div>
                <span className="lvn-lichad-da text-sm text-[#596370]">
                  15/1
                </span>
              </a>
            </div>
          </div>
        </div>
        <p className="font-semibold text-left text-lg mt-4">Chú thích</p>
        <div className="flex pl-2">
          <span className="good-day w-2/5 text-left">Ngày đẹp</span>
          <span className="ugly-day">Ngày xấu</span>
        </div>
        <p className="font-semibold text-left text-lg my-4">Ngày này năm xưa</p>
        <div className="flex pl-2">
          <img className="mr-2" src="/images/celebrate.png" alt="" />
          <div className="flex flex-col">
            <span className="font-normal text-sm text-[#596370] text-left">
              26/12/1991
            </span>
            <span className="font-medium	">
              Liên bang Liên Xô tuyên bố tan rã
            </span>
          </div>
        </div>
      </div>
    </section>
  )
}
